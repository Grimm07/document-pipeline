...
class ClassifierService:
    ...
    def classify(self, text: str, labels: list[str]) -> tuple[str, float, dict[str, float]]:
        ...
        Returns:
            Tuple of (top_label, confidence_score, all_scores). Returns
            ("unknown", 0.0, {}) if text is empty or whitespace-only.
        """
        ...
        all_scores = {
            lbl: float(sc) for lbl, sc in zip(result["labels"], result["scores"], strict=True)
        }
